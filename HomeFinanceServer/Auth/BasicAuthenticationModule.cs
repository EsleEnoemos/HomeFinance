...
namespace BasicAuth {
	...
	public class BasicAuthenticationModule : IHttpModule {
		private static IAuthProvider AP {
			get {
				if( _aP == null ) {
					...
					_aP = Activator.CreateInstance( providerType, false ) as IAuthProvider;
				}
				return _aP;
			}
		}
		private static IAuthProvider _aP;
		...
		static BasicAuthenticationModule() {
			...
			//string provider = ConfigurationManager.AppSettings[ "CustomAuthenticationProvider" ];
			//Type providerType = Type.GetType( provider, true );
			//AP = Activator.CreateInstance( providerType, false ) as IAuthProvider;
		}
		...
		private bool TryAuthenticate( HttpApplication context ) {
			...
			if( !string.IsNullOrEmpty( authHeader ) ) {
				if( authHeader.StartsWith( "basic ", StringComparison.InvariantCultureIgnoreCase ) ) {
					...
					IBasicUser bu;
					if( AP.IsValidUser( parts[ 0 ], parts[ 1 ], out bu ) ) {
